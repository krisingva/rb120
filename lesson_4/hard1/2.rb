...
# Answer: Move the method `range` from `WheeledVehicle` class into a module
# `FuelStuff` and include this module in `WheeledVehicle` and `Catamaran`
# classes.
...
# module (let's call Moveable) that we include in both individual classes.
...
# capacity and efficiency) using self. notation. Otherwise, this would be

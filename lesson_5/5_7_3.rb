...
# `activate_defense?` method in `Board` class that will determine if there are
# two human marks in a winning line (using a new `two_human_markers?` private
# method). The `two_human_markers? will only return true if in a winning line
# there are only two squares that are marked with human mark and an empty
# square. If `activate_defense?` returns true, call `computer_defense` that will
# also call `two_human_markers?` and find the square that is empty.
